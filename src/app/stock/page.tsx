...
export default function StockPage() {
  ...
  const columns = [
    { key: 'productName', name: t('stock.productName'), width: 200, editable: true },
    ...
    { key: 'minimumQuantity', name: t('stock.minimumQuantity'), width: 120, editable: true },
    { key: 'currentStock', name: t('stock.currentStock'), width: 120, editable: true },
  ]

  const allowedRestockFrequencies = ['daily', 'weekly', 'monthly', 'custom'] as const;
  ...
  const handleDataChange = useCallback((newData: any[]) => {
    ...
    setStockItems(
      newData.reduce((acc: StockItem[], row) => {
        const rf = isRestockFrequency(row.restockFrequency) ? row.restockFrequency : 'weekly';
        const item = {
          ...row,
          associatedProviders: typeof row.associatedProviders === 'string' 
  ...
  const handleDeleteRows = useCallback((rowsToDelete: any[]) => {
    ...
    setStockItems(stockItems.filter(item => !idsToDelete.includes(item.id)))
  }, [stockItems])
    ...
    reader.onload = (e) => {
      ...
      const importedItems = lines.slice(1).map((line, index) => {
        ...
        return {
          ...
          associatedProviders: values[7] ? values[7].split(';').map(p => p.trim()) : [],
  ...
  const lowStockItems = stockItems.filter(item => 
    (item.currentStock || 0) <= (item.minimumQuantity || 0)
  )
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-700">
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li>Link products to multiple providers for backup options</li>
